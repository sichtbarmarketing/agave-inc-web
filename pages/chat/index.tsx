...
import messageConverter, {
	NewMessage,
} from "@firebaseUtils/client/messageConverter";
import chatConverter, { NewChat } from "@firebaseUtils/client/chatConverter";
import profileConverter, {
	Profile,
} from "@firebaseUtils/client/profileConverter";
...
const ChatIndexPage: NextPage<ChatIndexProps> = () => {
	...
	const messagesCol = collection(db, `chats/${chatId}/messages`).withConverter(
		messageConverter
	);
	...
	const addMessage = async (
		message: NewMessage,
		msgCol: CollectionReference,
		chatDoc: DocumentReference
	) => {
		try {
	...
	const addChat = async (
		prof: Profile
	): Promise<{ ref?: DocumentReference; err?: Error }> => {
		let resObj: { ref?: DocumentReference; err?: Error } = {};

		if (!AuthUser.id)
			return { ...resObj, err: new Error("Not logged in: No user found!") };
		if (!prof.id) return { ...resObj, err: new Error("No Profile found!") };
		if (prof.id === AuthUser.id)
			return { ...resObj, err: new Error("That's you!") };
		...
		try {
			...
			if (!prevChats.empty)
				resObj = {
					...resObj,
					err: new Error("Previous chats found; Creating new chat anyway."),
				};

			const chat: NewChat = {
				createdBy: AuthUser.id,
				members: [AuthUser.id, prof.id],
				title: `${AuthUser.displayName} & ${prof.displayName}`,
				icon: null,
				lastMessage: {
					sender: AuthUser.id,
					timestamp: serverTimestamp(),
	...
	return (
		<>
			<Stack
				...
				<ChatListContainer
					flex={"1 0"}
					sx={{ minWidth: { xs: "100%", md: "30%" }, maxWidth: { md: 300 } }}
			...
			<LocationDialog
				send={handleSend}
				toggleDialog={toggleLocationDialog}
				open={locationDialog}
			/>
			<CreateDialog
				list={profiles}
