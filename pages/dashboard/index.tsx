...
const DashboardPage: NextPage = () => {
	...
	const fetcher = useSWR(AuthUser.id ? propertiesURL : null, async () => {
		...
		return await axios
			.get(propertiesURL, { headers: { Authorization: token } })
			.then((res) => res.data)
			.catch((e) => {
				console.error(e);
				throw e;
			...
			<AuthLayout signedIn={!!AuthUser.id} displayName={AuthUser.displayName}>
				<Container maxWidth='md' sx={{ mt: 3, mb: 6, minHeight: "50vh" }}>
					<TitleBar
						...
						Title={
							AuthUser?.displayName ? `Hello, ${AuthUser.displayName}!` : "Dashboard"
						}
					/>
					<Stack
						direction='row'
						...
						<Stack
							...
							<UserCard
								...
								sx={{
									minWidth: { xs: "100%", sm: "calc(50% - 1em)", md: "calc(60% - 1em)" },
								}}
							/>

							<Box
								minWidth={{ xs: "100%", sm: "calc(50% - 1em)", md: "calc(40% - 1em)" }}
								maxWidth={"100%"}
								position='relative'>
