...
const PropertyFormPage: NextPage<PropertyFormProps> = () => {
	...
	const { value: crossStreets } = useFormParser<string>(
		"Cross Streets",
		"text",
		formData?.fields
	);
	const { value: jobNumber } = useFormParser<string>(
		"Job Number",
		"text",
		formData?.fields
	);
	const { value: completedBy } = useFormParser<string>(
		"Completed by",
		"value",
		formData?.fields
	);
	const { value: completionDate } = useFormParser<string>(
		"Completion Date",
		"value",
		formData?.fields
	);

	const { value: materials } = useFormParser<string>(
		"Materials Total",
		"value",
		formData?.fields
	);
	const { value: labor } = useFormParser<string>(
		"Labor Total",
		"value",
		formData?.fields
	);
	const { value: total } = useFormParser<string>(
		"Grand Total",
		"value",
		formData?.fields
	);
