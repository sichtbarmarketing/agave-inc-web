...
const PropertiesPage: NextPage<PropertyPageProps> = () => {
	...
	const handleTemplate = (e: SyntheticEvent, value: TemplateID) => {
		setTemplate(value);
	};
	const searchControl = (event: ChangeEvent<HTMLInputElement>) => {
	...
	let property: undefined | null | yup.InferType<typeof PropertySchema> =
		data?.property;
		...
		<AgaveLayout>
			<AuthLayout signedIn={!!AuthUser.id} displayName={AuthUser.displayName}>
				<FilterContext.Provider
					value={{ template: template, dateRange: dateRange, search: search }}>
					<Box px={2} mb={4}>
						<TitleBar
							TitleIcon={WorkHistoryIcon}
							Title={`${property?.name ?? "Property"} - Work History`}
						/>
							...
							<Stack flex='3 0' sx={{ gap: "2em" }}>
								...
								<Box>
									...
									<WorkHistoryFilters
										...
										to={dateRange.end}
										calendarControl={toggleCalendar}
									/>
									...
									<Box overflow='hidden'>
										...
										<List disablePadding>
											<AgaveDetails
												...
												secondary={property?.accountMgr?.name}
												email={property?.accountMgr?.email}
												phone={
													property ? property.accountMgr?.phone?.toString() : undefined
												}
												storage={storage}
												imgURL={property?.accountMgr.image}
											/>
											<AgaveDetails
												Icon={PersonOutlineOutlinedIcon}
												primary={"Community Manager"}
												secondary={property?.manager?.name}
												email={property?.manager?.email}
											/>
											<AgaveDetails
												Icon={NumbersOutlinedIcon}
												primary={"Job Number"}
												secondary={property?.jobNumber}
											/>
										</List>
									</Box>
...
function WorkHistoryList({ propertyId, AuthUser }: WorkHistoryListProps) {
	...
	const fetcher = useSWR(
		AuthUser.id && propertyId && template && dateRange.start && dateRange.end
			? [
					historyURL,
					propertyId,
					template,
	...
	if (!propertyId || isLoading || !data)
		return (
			<WorkHistoryAlert
				Icon={<CircularProgress sx={{ mb: 3 }} />}
	...
	return (
		<Stack
			...
			{data?.forms.length == 0 && (
				<WorkHistoryAlert
					Icon={<ErrorIcon sx={{ fontSize: "3rem", mb: 3 }} />}
					title='No Work History Available'
					text='Contact us if you need assistance.'
				/>
			)}
