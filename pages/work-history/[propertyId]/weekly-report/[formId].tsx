                ...
                <Stack direction='row' flexWrap='wrap' justifyContent='center' sx={{ gap: '1em' }}>
                    <Box flex='1 0' maxWidth={{ xs: '100%', lg: 'calc(50%)' }} minWidth={300}>
                        <Stack direction='row' justifyContent='space-between' alignItems='center' px={2}>
                            <TitleBar TitleIcon={WorkHistoryOutlinedIcon} Title={'Weekly Report'} flexGrow={0}/>
                            <Button color='secondary' variant='outlined' disabled startIcon={<IosShareOutlinedIcon/>}>
                                Share
                            </Button>
                        </Stack>
                        <Divider />
                    ...
                    <Box flex='1 0' maxWidth={{ xs: '100%', lg: 'calc(50% - 1em)' }} m={2} minWidth={300}>
                        <ImageList sx={{borderRadius: 3}}>
                            {details
                                .filter(({ image }: WeeklyReportDetails) => (image && image.imageFile && image.id))
                                .map(({image, text}: WeeklyReportDetails) =>
                                <WeeklyReportDetails key={image.id} image={image} text={text} user={AuthUser} />
                            )}
                        </ImageList>
                    </Box>
                </Stack>
