...
interface AgaveLayoutProps {
	user?: User;
	children?: ReactNode;
	bgMain?: BoxProps["bgcolor"];
}
const AgaveLayout: FC<AgaveLayoutProps> = ({
	bgMain = "grey.300",
	user,
	children,
}: AgaveLayoutProps) => {
    ...
    const chatData = useChatData(null);
			...
			<NavigationRail width={drawerWidth} lWidth={lWidth}>
				<Stack
					...
					<NavButton
						...
						curr={"/chat" === pathname}
						href="/chat"
						showNotification={pathname !== "/chat" && chatData.hasUnreadChatMessages}
					/>
					<NavButton
